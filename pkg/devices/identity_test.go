...
// TestIdentityNilReadingValue tests that we get a nil reading value for a
// current reading where the OID is not served by the emulator. See
// https://github.com/vapor-ware/synse-snmp-plugin/issues/61
func TestIdentityNilReadingValue(t *testing.T) {
	// Create an identity device that will give us a nil reading value.
	device := sdk.Device{
		Data: map[string]interface{}{
			...
			//
			"base_oid":   ".1.3.6.1.2.1.32.1.1.%d.0", // ups mib is at .33, not .32
			"column":     4,
			"oid":        ".1.3.6.1.2.1.32.1.1.4.0", // ups mib at .33, not .32
			"row":        0,
			"table_name": "UPS-MIB-UPS-Identity-Table",
		},
	}
	// Call SnmpIdentityRead.
	readings, err := SnmpIdentityRead(&device)
