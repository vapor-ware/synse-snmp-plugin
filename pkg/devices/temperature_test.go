...
// TestTemperatureNilReadingValue tests that we get a nil reading value for a
// current reading where the OID is not served by the emulator. See
// https://github.com/vapor-ware/synse-snmp-plugin/issues/61
func TestTemperatureNilReadingValue(t *testing.T) {
	// Create a temperature device that will give us a nil reading value.
	device := sdk.Device{
		Data: map[string]interface{}{
			...
			"column":     7,
			"oid":        ".1.3.6.1.2.1.32.1.2.7.0", // ups mib at .33, not .32
			"row":        0,
			"table_name": "UPS-MIB-UPS-Battery-Table",
		},
	}
	// Call SnmpTemperatureRead.
	readings, err := SnmpTemperatureRead(&device)
